...
function AppCard({ app }: { app: TutorialApp }) {
  ...
  const videoUrl = settings.tutorialVideos?.[app.key] || '';
      ...
      <div className="p-5 sm:p-6">
        <div className="flex items-center gap-4 mb-4">
          ...
          <a href={app.downloadUrl} target="_blank" rel="noopener noreferrer"
            className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-brand-500 to-cyan-500 text-white font-semibold rounded-xl text-sm hover:shadow-lg hover:shadow-brand-500/25 transition-all hover:scale-105 shrink-0">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {app.features.map((f, i) => (
            <span key={i} className="text-xs px-2 py-1 rounded-lg dark:bg-gray-700/50 bg-gray-100 dark:text-gray-300 text-gray-600">{f}</span>
          ))}
        </div>

        {/* Video embed */}
        {videoUrl && <VideoEmbed url={videoUrl} title={`${app.name} Tutorial`} />}
        ...
        {expanded && (
          <div className="mt-4 animate-fade-in border-t dark:border-gray-700 border-gray-200 pt-4">
            <h4 className="font-semibold dark:text-white text-gray-900 mb-3">Step-by-Step Setup:</h4>
            <ol className="space-y-3">
              {app.steps.map((step, i) => (
                <li key={i} className="flex gap-3">
                  <span className="shrink-0 w-6 h-6 rounded-full bg-brand-500/20 text-brand-400 text-xs font-bold flex items-center justify-center mt-0.5">{i + 1}</span>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="text-center mb-12">
          ...
          <p className="dark:text-gray-400 text-gray-600 max-w-xl mx-auto text-lg">
            Follow our step-by-step guides and video tutorials to configure your V2Ray client.
          </p>
          <button onClick={() => navigate('/vpn')} className="mt-4 text-brand-400 font-medium hover:text-brand-300 transition-colors inline-flex items-center gap-1">
        ...
        <section className="mb-12">
          ...
          <div className="space-y-4">
            {windowsApps.map(app => <AppCard key={app.key} app={app} />)}
          </div>
        </section>
        ...
        <section className="mb-12">
          <div className="flex items-center gap-3 mb-6">
            <div className="w-10 h-10 rounded-xl bg-green-500/20 flex items-center justify-center"><Smartphone className="w-5 h-5 text-green-400" /></div>
            <div>
          ...
          <div className="space-y-4">
            {androidApps.map(app => <AppCard key={app.key} app={app} />)}
          </div>
        </section>
        ...
        <section className="mb-12">
          <div className="flex items-center gap-3 mb-6">
            <div className="w-10 h-10 rounded-xl bg-gray-500/20 flex items-center justify-center"><span className="text-lg">🍎</span></div>
            <div>
          ...
          <div className="space-y-4">
            {iosApps.map(app => <AppCard key={app.key} app={app} />)}
          </div>
        </section>
